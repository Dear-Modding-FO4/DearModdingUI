...
namespace DearModdingUI
{
	...
	struct TreeSidebarLayout
	{
		...
		static void Activate(
			...
			SidebarBrowsingState& a_state)
		{
			RevealSelection(a_model, a_selection, a_state);
		}

		template<class Context>
		static void Draw(const Context& a_context) noexcept
		{
			a_context.DrawTree();
		}
	};
		...
		SidebarBrowsingState& a_state)
	{
		VisitSidebarLayout(
			a_kind,
			[&]<class Layout>() {
				Layout::Activate(a_model, a_selection, a_state);
			});
	}
