...
namespace DearModdingUI
{
	...
	struct NavigationSearchRecord
	{
		...
		std::string normalizedClientDisplayName;
	...
	{
		const NavigationSearchRecord* record{};
		NavigationMatchQuality match{ NavigationMatchQuality::kSummary };

		[[nodiscard]] const NavigationSearchEntry& Entry() const noexcept
		{
			return record->entry;
		}
	};
	...
	struct NavigationModel
	{
		...
		[[nodiscard]] std::span<const NavigationSearchRecord>
			SearchIndex() const noexcept
		{
			return m_searchIndex;
		}

	private:
		std::vector<NavigationSearchRecord> m_searchIndex;
	};
