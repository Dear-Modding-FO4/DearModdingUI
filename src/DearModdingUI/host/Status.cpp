...
{
	const ClientStatus* FindClientStatus(
		...
		DMUI_ClientHandle a_client) noexcept
	{
		const auto found = std::ranges::find(
			...
			a_client,
			&ClientStatus::client);
		return found == a_statuses.end() ? nullptr : std::to_address(found);
	}
		...
		StatusClock::time_point a_now) noexcept
	{
		...
		try
		{
			...
			static_assert(std::is_nothrow_move_constructible_v<StatusMessage> &&
				std::is_nothrow_move_assignable_v<StatusMessage>);
			if (a_client != DMUI_INVALID_CLIENT_HANDLE)
			{
				// Prepare the only extra copy before committing either published status.
				auto clientStatus = next;
				const auto found = std::ranges::find(
					m_clientStatuses,
					a_client,
					&std::pair<DMUI_ClientHandle, StatusMessage>::first);
				if (found == m_clientStatuses.end())
					m_clientStatuses.emplace_back(a_client, std::move(clientStatus));
				else
					found->second = std::move(clientStatus);
			}
