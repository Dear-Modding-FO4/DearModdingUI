...
def mutation_bytes(entry: dict[str, str], original: bytes) -> tuple[bytes, bytes]:
    find_text = entry["find"].replace("\r\n", "\n").encode("utf-8")
    replacement_text = entry["replace"].replace("\r\n", "\n").encode("utf-8")
    endings = (b"\r\n", b"\n") if b"\r\n" in original else (b"\n", b"\r\n")
    candidates: dict[bytes, bytes] = {}
    for ending in endings:
        find = find_text.replace(b"\n", ending)
        candidates.setdefault(find, replacement_text.replace(b"\n", ending))
    occurrences = sum(original.count(find) for find in candidates)
    if occurrences != 1:
        raise MutationFailure(
    ...
    return next(
        (find, replacement)
        for find, replacement in candidates.items()
...
def run_mutation(entry: dict[str, str], index: int, total: int) -> None:
    ...
    try:
        ...
        if restoration_required:
            restore_target(target, original, mutated, entry, find, replacement)
