...
namespace DearModdingUI::MCM
{
	...
	void BindActions(
		...
		DiagnosticReporter& a_diagnostics)
	{
		std::shared_ptr<PageActionState> state;
		const auto bindState = [&]() {
			...
			state = std::make_shared<PageActionState>(
				std::move(bindings),
				a_diagnostics);
		...
		for (const auto& row : a_page.rows)
		{
			if (!row.action)
				continue;
			if (auto* action = FindActionRow(a_page.settings, row.id))
			{
				if (row.actionInertReason)
					continue;
				auto actionState = bindState();
				action->activate =
					[&a_executor,
					 actionValue = *row.action,
					 id = row.id,
					 state = std::move(actionState)] {
						Invoke(
							a_executor,
							actionValue,
							std::nullopt,
			...
			setting->binding.set =
				[&a_executor,
				 ...
				 id = row.id,
				 state = std::move(actionState)](
					dmui::SettingValue a_value) mutable {
					if (writeValue)
					{
